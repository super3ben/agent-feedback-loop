...
## Task 2 v2: Immediate Reviewer Job Control (2026-07-20)

### Status

DONE_WITH_CONCERNS. The immediate candidate, launch reservation, recoverable listing, fenced lease,
bounded context, retry, and terminal transition APIs are implemented without adding a launcher or
making the main session wait.

### TDD Evidence

- Required RED command: `node --test --test-name-pattern='candidate identity is replay-idempotent|different sessions are never text-deduplicated|launch reservation is bounded|stale reviewer owners cannot publish' test/control-store.test.mjs`
  produced 0 passed and 4 failed. Every failure was the expected missing `createReviewCandidate` API.
- Additional recoverable-cooldown RED: 0 passed and 1 failed because a launch-reserved job remained
  visible in the recoverable list.
- Focused Task 2 GREEN: 8 passed and 0 failed.
- Required regression GREEN: `node --test test/control-store.test.mjs test/store.test.mjs test/capture.test.mjs`
  produced 159 passed and 0 failed.
- `node --check src/control-store.mjs`, `node --check test/control-store.test.mjs`, and
  `git diff --check` exited 0.

### Behavior And Boundaries

- Candidate replay is idempotent only when source event, referent event, and project binding match;
  contradictory reuse of a stable source identity fails closed.
- Every state mutation runs in one `BEGIN IMMEDIATE` transaction. Claim, reclaim, renewal,
  completion, failure, exhaustion, and launch release all use conditional SQL fences.
- Jobs are immediately pending. Launch reservations increment `launch_epoch`; only the current epoch
  can release a synchronous launch failure. Recoverable jobs are due, stable by `created_at, job_id`,
  and hard-capped at eight.
- Claims use a 185-second default lease, increment attempt and lease epoch, allow only expired-running
  reclaim, and fence stale owners. Retryable failures wait 30 then 120 seconds; attempt three is terminal.
- Reviewer context contains only bounded metadata, opaque event identifiers, content hashes, and
  encrypted blob references. Decryption remains runner-owned; no prompt, report, method, lesson, card,
  notification, scheduler, or Markdown body path was added.
- No schema/version, capture, installer, CLI, launcher/provider, hook, notification, RAG, real HOME,
  real runtime, or real database was modified.

### Concerns And Unverified Boundaries

- The owned diff is intentionally API-heavy (eleven required store methods plus tests), so SQL/state
  transition review is the primary residual risk; no generic job framework was introduced.
- Verification is local macOS Node/SQLite with disposable HOME/database fixtures. Detached process,
  provider, Linux, live hook, installation, and production behavior remain unverified and out of scope.

## Task 2 v2 Frozen Review-Fix 1: Exhausted Lease Finalization

### Status

DONE_WITH_CONCERNS. The single frozen finding is fixed without changing the Task 2 API or adding a
launcher, scheduler, schema, provider, service, or sweep framework.

### RED And GREEN Evidence

- Focused RED: `node --test --test-name-pattern='bounded recovery finalizes an expired third reviewer attempt exactly once' test/control-store.test.mjs` produced 0 passed and 1 failed because the expired third-attempt row was absent from the bounded recoverable list.
- Focused GREEN: the same command produced 1 passed and 0 failed.
- Task 2 focused GREEN: nine Task 2 job-control tests produced 9 passed and 0 failed.
- Required regression GREEN: `node --test test/control-store.test.mjs test/store.test.mjs test/capture.test.mjs` produced 160 passed and 0 failed.
- Both `node --check` commands and `git diff --check` exited 0.

### Minimal Fix

- The bounded listing now exposes due expired-running rows even at attempt three while still keeping
  pending/retryable claims below the attempt limit.
- `reserveReviewLaunch` conditionally terminalizes only an expired running job at or above the limit
  inside its existing `BEGIN IMMEDIATE` transaction. It clears owner, lease, retry and next-launch
  state, sets `completed_at` and `error_code='attempts_exhausted'`, and inserts one fenced `failed`
  event using the final lease epoch.
- Repeated listing, reservation and claim calls cannot reopen the terminal job or duplicate the event.

### Unverified Boundaries

- Evidence remains local macOS Node/SQLite with disposable HOME/database fixtures. True concurrent
  multi-process stress, detached launch, provider, Linux, live hook, installation and production
  behavior remain outside this frozen fix.
