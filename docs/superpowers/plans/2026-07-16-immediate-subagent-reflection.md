...
**Frozen Task 12 preflight decisions:**

- This is an explicit offline export tool, not a legacy read path for prompt selection or reviewers. Support only legacy schema versions 8 and 9, and build the new test's minimal fixtures locally so Task 13 can delete the old runtime fixture without breaking export coverage.
- `sourceDb` must be an absolute canonical path to a current-user-owned regular file with no symlink component. The source set is that file plus existing `-wal` and `-shm` siblings. Open every member with no-follow semantics, hash the set before and after copying, copy it into a private temporary directory, and reject any identity/content/source-set change. Open only the private snapshot with `DatabaseSync(..., { readOnly: true })`, issue fixed literal `SELECT` statements for the schema version and four declared tables, close it, and remove the snapshot. Do not import `openStore`, execute PRAGMA/migration/DDL/DML, or read any other legacy table.
- One export candidate is one lesson entry in a committed `review_receipts.payload_json`, keyed by receipt id, lesson id and revision. `reviewed_no_lesson` receipts produce no candidate. Join reports, lesson rows and revisions in JavaScript by their persisted ids/job binding. Malformed JSON, missing/mismatched report or revision, unsupported severity/responsibility, invalid timestamps, incomplete card/evidence/causal fields, or any canonical model bound failure produces one opaque incomplete id; never infer missing content.
- Reconstruct the original family deterministically as SHA-256 of the persisted legacy `method_class`, a NUL separator and `class_id`; normalize the legacy hyphenated method class to the new underscore form only after hashing. Map `card.when` to `applies_when`, exact feedback quotes to facts and the plain-language complaint, the persisted redacted report body to root cause, `class_id` to class of mistake, and the bounded required/forbidden/verify card fields to method changes. Legacy export has no qualifying new emission evidence, so effectiveness is `unknown` and repeated-pattern evidence is empty.
- Derive an opaque `legacy-<hash>` id from the receipt/lesson/revision tuple and bind it to a SHA-256 of the exact four-row evidence used for conversion; that combined identity is the source envelope passed through the existing `validateReflectionModel`. Results and CLI output expose only counts, opaque legacy ids and outcome/reason codes, never receipt/report/card bodies or raw database ids.
