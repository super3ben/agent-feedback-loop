...
store 中。该 binding 是剥离协议的权威字段，不是展示用随机标签。

新建 `notification_id`、renderer 和 hidden marker 只接受小写 64-hex canonical ID。Stop observation 为读取历史
store 行可额外接受唯一已证明的 legacy 语法 `notification-<positive integer>`；不接受 UUID、`msg_UUID`、
...
进入 capture/reconcile 前，剥离器只删除相邻的完整控制对，并同时验证 canonical marker ID、state、nonce、
state-specific 可见文法以及 `receipt=<marker id 前 6 位>`。反引号或波浪号 fenced code block 内的内容、quoted
内容、畸形 marker、错误 state/nonce/binding、旧版无 binding 行和非相邻控制必须原样保留，避免删除用户证据。
...
11. 真实 outbox 行只有 canonical 64-hex 或精确 legacy `notification-<positive integer>` marker 可转为 `observed`；UUID、`msg_UUID`、session 和 path 形态保持未确认。
