...
每个 task + boundary + canonical invariant 一行，保存 lifecycle、failure count、fix generation、decision basis digest、current decision、direction generation、bounded alias ID list、active grant ID 与版本号。closed 后再次失败更新同一行，不创建可逃逸的新身份。
...
`contract_projected`、`generation_opened`、`evidence_recorded`、`review_recorded`、`alias_declared`、`distinct_declared`、`breaker_triggered`、`reflection_requested`、`reflection_completed`、`checkpoint_recorded`、`grant_issued`、`grant_consumed`、`grant_revoked`、`generation_closed`、`task_resolved`、`legacy_imported`、`shadow_compared`。
