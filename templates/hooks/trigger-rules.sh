...
agent_feedback_should_force_reflect() {
  ...
  force_pattern='非常不满意|生气|严重问题|严重.*现场事故|现场事故|自我反思|反思了吗|触发.*反思|不要询问.*要不要|不要问.*要不要|别问.*要不要|默认就要|大问题|太差|重做|重来|没有按规则|没按规则|没有真机测试|没真机测试|critical|blocker|very dissatisfied|angry|furious|unacceptable|serious issue|major issue|production incident|site incident'
...
agent_feedback_gate_message() {
  prompt_file="$1"
  marker_path="$2"
  printf '反馈检查：请语义检查最新用户消息。如果消息表达不满、纠错、重复失败、流程质疑，或要求未来防复发规则（例如“怎么又/为什么还/又…了/不要再”这类重复出错或不满的口吻也算），你必须：(a) 先执行 `touch %s` 标记本轮需要反思；(b) 遵循 %s 完成反思，把完整报告写入项目内 .agent/reflections/<时间戳>.md（目录不存在就创建），回合里只留一行摘要、不要把整篇反思贴进对话；(c) 若结论是 agent_fault 且有证据、中高置信度、具备具体防复发约束，默认直接写入 .agent/rules/feedback-loop.md，无需再询问用户是否写入；(d) 在本轮回复结尾输出一行 `%s responsibility=<分类>-->` 作为机器可校验的完成凭据。否则忽略本检查并正常回答。反思报告默认使用中文，除非用户明确选择其他语言。' "$marker_path" "$prompt_file" "$AFL_DONE_MARKER"
}
...
agent_feedback_stop_reason() {
  prompt_file="$1"
  printf '检测到本轮要求反思但未见反思完成标记。请立即遵循 %s 完成反思（分类责任为 agent_fault/user_misunderstanding/shared_ambiguity/external_limit/insufficient_evidence）；若结论是 agent_fault 且有证据、中高置信度、具备具体防复发约束，默认直接写入 .agent/rules/feedback-loop.md，无需再询问用户是否写入；并在回复结尾输出一行 `%s responsibility=<分类>-->`。' "$prompt_file" "$AFL_DONE_MARKER"
}
